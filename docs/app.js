...
// Injected by GitHub Actions from GROQ_API_KEY repository secret at deploy time.
// Do NOT replace this placeholder manually in source code.
const DEFAULT_GROQ_KEY = '__GROQ_API_KEY__';

// Settings (persisted in localStorage — user can override key in Settings panel)
let groqApiKey      = localStorage.getItem('groq_api_key')    || DEFAULT_GROQ_KEY;
...
// Persist default key so the session uses it even before the user opens Settings
...
5. Keep your tone scientific and professional.`;
    ...
    async _groq(model, messages, opts = {}) {
        ...
        try {
            ...
            throw new Error(`Network error reaching Groq API. Check your internet connection. (${networkErr.message})`);
        }
        ...
        if (!res.ok) {
            ...
            if (res.status === 401) errMsg = 'Invalid Groq API key. Please update it in Settings.';
            if (res.status === 429) errMsg = 'Groq rate limit reached. Please wait a moment and try again.';
...
Return a JSON object ONLY (no markdown) with these exact fields:
{
  ...
  "verdict": "<PASS if score>=80, WARNING if 50-79, FAIL if <50>"
        ...
        if (!res.ok) {
            console.warn('Validator error:', res.status);
...
function init() {
    // Pre-populate settings with current values (including default key)
    ...
    // Only show settings modal if no key is configured at all
    // (won't trigger since DEFAULT_GROQ_KEY is always set)
    ...
    // Upload — both button and dropzone click trigger file picker
    const openFilePicker = (e) => {
        if (e) e.stopPropagation();
        D.pdfFileInput.value = '';
        D.pdfFileInput.click();
    };
    D.btnBrowse.addEventListener('click', openFilePicker);
    D.dropZone.addEventListener('click', openFilePicker);
    ...
    D.dropZone.addEventListener('dragleave', e => { e.stopPropagation(); D.dropZone.classList.remove('dragover'); });
